...
def inference(boxes, dataconfig, p_keep_conv, p_keep_hidden, l2scale=0.0, l2scale_conv=0.0):
    ...
    prev_layer = _convlayer(prev_layer, 48, "conv1", keep_prob=p_keep_conv, pooling=True, l2scale=l2scale_conv)

    prev_layer = _convlayer(prev_layer, 96, "conv2", keep_prob=p_keep_conv, pooling=True, l2scale=l2scale_conv)

    with tf.variable_scope('conv3_multi'):
        prev_layer = _convlayer(prev_layer, 192, "conv3_1", keep_prob=None, pooling=False, l2scale=l2scale_conv)
        prev_layer = _convlayer(prev_layer, 192, "conv3_2", keep_prob=None, pooling=False, l2scale=l2scale_conv)
        prev_layer = _convlayer(prev_layer, 96, "conv3_3", keep_prob=p_keep_conv, pooling=True, l2scale=l2scale_conv)

    prev_layer = _fc_layer(prev_layer, 3072, "local4", keep_prob=p_keep_hidden, l2scale=l2scale)
    prev_layer = _fc_layer(prev_layer, 3072, "local5", keep_prob=p_keep_hidden, l2scale=l2scale)
